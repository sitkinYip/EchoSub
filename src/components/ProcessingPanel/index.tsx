...
export default function ProcessingPanel({
  progressMessage, pipelinePhase, subtitleCount, onCancel, hasError, isVideoMode,
}: Props) {
  ...
  switch (pipelinePhase) {
    case "extracting": currentStep = 0; break;
    case "uploading": currentStep = 1; break;
    case "translating": currentStep = subtitleCount > 0 ? 3 : 2; break;
    default: currentStep = steps.length; break;
  }
  ...
  return (
    <div className="rounded-2xl bg-app-surface-alt ring-1 ring-app-border p-6 space-y-5">
      <div className="flex items-center justify-between">
        ...
        {pipelinePhase && (
          <button onClick={onCancel} className="px-2.5 py-1 text-[11px] text-app-error hover:text-app-error bg-app-error-bg hover:bg-app-error-bg rounded-lg transition-all">
            取消
          </button>
        )}
      </div>
          ...
          return (
            <div key={step.key} className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-medium transition-colors duration-300 ${done ? "text-app-success" : err ? "text-app-error" : active ? "text-app-text" : "text-app-text-tertiary"}`}>
                  {step.label}
                </p>
                <p className={`text-xs mt-0.5 ${err ? "text-app-error" : "text-app-text-tertiary"}`}>
