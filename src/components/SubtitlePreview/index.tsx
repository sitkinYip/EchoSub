...
export default function SubtitlePreview({ items, onUpdateText, contained = true }: Props) {
  ...
  const doubleClick = useCallback((item: SubtitleItem) => {
  ...
  const save = useCallback(
    (idx: number) => {
      if (editText.trim()) onUpdateText(idx, editText.trim());
      setEditIdx(null);
    },
    ...
    <div className="rounded-2xl bg-app-surface-alt ring-1 ring-app-border overflow-hidden">
      ...
      >
        {items.map((item, i) => (
          <div
            ...
            className="flex items-center px-5 py-2.5 hover:bg-app-surface transition-colors group"
          >
            <span className="w-8 text-xs text-app-text-tertiary font-mono">{item.index}</span>
            <span className="w-44 text-xs text-app-text-tertiary font-mono tabular-nums">
              {item.start} → {item.end}
            </span>
            <div className="flex-1 min-w-0 cursor-pointer" onDoubleClick={() => doubleClick(item)}>
              {editIdx === item.index ? (
                ...
                <p className="text-sm text-app-text-secondary truncate group-hover:text-app-text transition-colors">
                  {item.text}
                </p>
              )}
            </div>
            {editIdx !== item.index && (
              <span className="ml-2 text-[10px] text-app-text-tertiary opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                双击编辑
              </span>
            )}
          </div>
      ...
      <div className="px-5 py-2.5 border-t border-app-border-light bg-app-surface-alt">
        <p className="text-xs text-app-text-tertiary">
          共 <span className="text-app-text-secondary font-medium">{items.length}</span> 条字幕 ·
          双击编辑
        </p>
      </div>
    </div>
