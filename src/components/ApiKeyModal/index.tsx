  ...
  return (
    <div>
      ...
      <div className="space-y-5">
        <div>
          <label className="block text-xs font-medium text-app-text-secondary mb-1.5 tracking-wide uppercase">DashScope API Key</label>
          <input
        ...
        <div className="flex items-center justify-between px-3 py-2.5 bg-app-surface-alt rounded-xl ring-1 ring-app-border-light">
          ...
          >
            <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${upload ? "translate-x-6" : "translate-x-1"}`} />
          </button>
        </div>
        <div className="text-xs text-app-text-tertiary space-y-1">
          <p>
            获取 API Key：{" "}
            <button type="button" onClick={() => shellOpen("https://dashscope.aliyun.com/")} className="text-app-accent hover:underline bg-transparent border-none p-0 text-xs cursor-pointer">
              DashScope 控制台
            </button>
          </p>
        ...
        <div className="flex gap-3 pt-1">
          ...
          <button
            onClick={handleSave} disabled={!key.trim() || saving}
