...
export default function TranslatePage() {
  ...
  const { appStep, pipelinePhase, videoFile, progress, error, subtitleCount,
    rawPreviewText, subtitleItems,
    startPipeline, reset, updateSubtitleText } = t;

  const [settingsOpen, setSettingsOpen] = useState(false);
  const pendingRef = useRef<VideoFile | null>(null);
  const modeLabel = uploadVideo ? "视频" : "音频";

  useEffect(() => { if (!loaded) s.load(); useHistoryStore.getState().load(); }, []);

  const doStart = useCallback((fp: string, fn: string, forceMode?: "audio" | "video") => {
  ...
  const onFile = useCallback((fp: string, fn: string) => {
    ...
    if (!ALL_SUPPORTED_EXTS.includes(ext)) {
      showMessage({ type: "error", title: "不支持的文件格式", description: `".${ext}" 不在支持的格式列表中，请选择视频或音频文件。` });
      return;
    }
    ...
    if (!apiKey) {
      ...
      showModal("ApiKey", {
        onCancel: () => { pendingRef.current = null; },
        onSaved: async (k: string, src: any, tgt: any, uv: boolean) => {
          await update({ apiKey: k, sourceLang: src, targetLang: tgt, uploadVideo: uv });
          const p = pendingRef.current;
          if (p) { pendingRef.current = null; doStart(p.path, p.name); }
        },
      });
      return;
    }
    ...
    <div className="flex flex-col h-full">
      <div className="px-8 pt-8">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 ml-auto">
            <LangSelect value={sourceLang} onChange={(l) => update({ sourceLang: l })} options={LANGUAGES} />
            <Icon name="chevron-right" className="w-3.5 h-3.5 text-app-text-tertiary flex-shrink-0" />
            <LangSelect value={targetLang} onChange={(l) => update({ targetLang: l })} options={LANGUAGES} />
            ...
            <div className="flex items-center">
              ...
              <div className="relative ml-1.5 group">
                <Icon name="help" className="w-3.5 h-3.5 text-app-text-tertiary/40 hover:text-app-text-tertiary cursor-help transition-colors" />
            ...
            <button onClick={() => setSettingsOpen(!settingsOpen)} className="relative ml-2 w-8 h-8 rounded-xl bg-app-surface hover:bg-app-hover ring-1 ring-app-border-light flex items-center justify-center transition-all duration-200 active:scale-95">
              ...
              {!hasApiKey && <span className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-app-error" />}
            </button>
        ...
        <div className="max-w-3xl mx-auto pb-24">
          {appStep === "idle" && (
            <div className="flex items-center justify-center min-h-[300px]">
              <div className="w-full max-w-2xl"><DropZone onFileSelect={onFile} /></div>
            </div>
          )}
          ...
          {appStep === "processing" && (
            <div className="space-y-6">
              {videoFile && (
                <FilePill name={videoFile.name} sourceLang={sourceLang} targetLang={targetLang} mode={modeLabel} onReset={reset} />
              )}
              <ProcessingPanel
              ...
              {pipelinePhase === "translating" && rawPreviewText && !subtitleItems.length && (
                <div className="rounded-2xl bg-app-surface-alt ring-1 ring-app-border p-5 max-h-64 overflow-y-auto">
                  <p className="text-xs text-app-text-tertiary mb-3 font-medium tracking-wide uppercase">实时流</p>
                  <pre className="text-sm text-app-text-secondary font-sans whitespace-pre-wrap break-words leading-relaxed">{rawPreviewText}</pre>
                </div>
              )}
        ...
        <div className="flex-shrink-0 px-8 py-4 bg-app-bg/80 backdrop-blur-xl border-t border-app-border-light">
          <div className="flex items-center justify-between max-w-3xl mx-auto">
            <span className="text-xs text-app-text-tertiary">
              {appStep === "processing" ? (error ? "处理出错" : "处理中...") : `${subtitleItems.length} 条字幕`}
            </span>
            <ExportButton items={subtitleItems} disabled={appStep !== "preview"} videoFileName={videoFile?.name} />
          </div>
        </div>
      )}
      ...
      {settingsOpen && (
        <div className="fixed inset-0 z-40" onClick={() => setSettingsOpen(false)}>
          <div className="absolute top-20 right-8 w-80 rounded-2xl bg-app-elevated ring-1 ring-app-border shadow-2xl p-5 z-50" onClick={(e) => e.stopPropagation()}>
            <SettingsPopover
              sourceLang={sourceLang} targetLang={targetLang}
              onSourceLangChange={(l) => update({ sourceLang: l })} onTargetLangChange={(l) => update({ targetLang: l })}
              apiKey={apiKey} onApiKeyChange={(k) => update({ apiKey: k })}
