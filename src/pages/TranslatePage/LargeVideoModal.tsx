    ...
    <div>
      <div className="flex items-start gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-xs text-app-text-tertiary mt-0.5 truncate max-w-[260px]">{data.videoName}</p>
        </div>
      </div>

      <div className="space-y-3 mb-5">
        <div className="px-3 py-2.5 bg-app-surface-alt rounded-xl ring-1 ring-app-border-light">
          <p className="text-sm text-app-text">文件大小：<span className="font-medium text-app-accent">{data.sizeMB}</span></p>
          <p className="text-xs text-app-text-tertiary mt-1">超过云端 1GB 上传限制，需压缩处理后上传</p>
        </div>

        <div className="px-3 py-2.5 bg-app-surface-alt rounded-xl ring-1 ring-app-border-light">
          <div className="flex items-start gap-2">
            <Icon name="spinner" className="w-3.5 h-3.5 text-app-text-tertiary mt-0.5 flex-shrink-0" />
      ...
      <div className="flex flex-col gap-2">
        ...
        <button
          onClick={() => { data.onSwitchToAudio(); close(); }}
