...
export function TabBtn({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex-1 px-3 py-2 text-xs font-medium rounded-lg transition-all duration-200 ${active ? "bg-app-surface text-app-text ring-1 ring-app-border" : "text-app-text-tertiary hover:text-app-text-secondary"}`}
    >
      {label}
      {count > 0 && (
        <span
          className={`ml-1.5 text-[10px] ${active ? "text-app-text-tertiary" : "text-app-text-tertiary/60"}`}
        >
          {count}
        </span>
      )}
...
function ListItem({
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left px-3 py-2.5 rounded-xl transition-all duration-200 mb-0.5 ${active ? "bg-app-surface ring-1 ring-app-border" : "hover:bg-app-surface-alt"}`}
    >
      <p
        className={`text-sm truncate font-medium ${active ? "text-app-text" : "text-app-text-secondary"}`}
      >
        {entry.videoName}
      </p>
